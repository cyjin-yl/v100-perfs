...
### 8. 遗留问题: IQ4_XS 的 decode 反而更慢

理论上 IQ4_XS 在带宽受限的 decode 上应当更快(每 token 少读约 26% 权重)。
实测相反: 约 5.9 tok/s vs Q5_K_M 的 6.9~12.1(非受控对比)。

算子路由普查给出了方向:

```
gguf.mmvq              1066667 次 / 1142922 tok   <- decode(n=1~3) 走这条
gguf.sm70_iq4xs_mmq       1904 次 /   87312 tok   <- 只在 n=13~59 触发
gguf.dequant_fp16_gemm   21616 次 / 19804306 tok  <- prefill
```

即**针对 IQ4_XS 的 SM70 MMQ 快路径在 decode 上根本没被调用**。所以这是
"没走到快路径", 不是"IQ4_XS 格式不行" —— 应该去优化 `mmvq` 的 IQ4_XS 分支,
而不是切回 Q5。
