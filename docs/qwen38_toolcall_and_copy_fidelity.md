...
# 续四：「Proto-UI 拼不对」的排查 —— 前面几个根因**都不是**它（2026-08-20）

这一节记录**被排除的假设**，因为不写下来，下次一定会有人（包括我自己）重新去追同一条死路。

## 结论先行

`Proto-UI` 被拼错这个现象，跟本文档前面修的**所有** bug 都无关。
那些 bug 各自真实存在、各自解决了各自的故障，但**没有一个是这个现象的原因**。
我一度把根因归到 MTP typical acceptance 上，那是过早了。

## 被排除的假设与判据

| 假设 | 判据 | 结论 |
|---|---|---|
| MTP typical acceptance 有损 | `FASTLLM_QWEN35_ENABLE_MTP=0` 后输出与 `=2` 时**逐字节相同** | **排除**。反过来这证明了 exact acceptance 修复是对的 —— "开关 MTP 输出必须相同"正是它的定义性质 |
| turbo3 KV 反量化路径 | 换 `--kv_cache_dtype float16` 后照错，且错法一致 | **排除** |
| NaN / 数值损坏 | 随机十六进制串抄得完全正确；同一输入 10 次输出**完全一致** | **排除**。数值噪声每次错法都不同，确定性错误不可能来自 NaN |
| 反分词器拼接 | 同一个词在不同上下文里**错法不同**（见下） | **排除**。反分词器是纯函数，同样的 token 序列必然拼出同样的结果，上下文相关的错误只能来自模型选了不同 token |

关键的那组对比：

```
brutalist  单独测   ->  "bru talist"   (插入空格)
brutalist  在路径里 ->  "buta-list"    (掉 r + 插连字符)
prototypes 单独测   ->  正确
prototypes 在路径里 ->  "proto-types"  (插入连字符)
Proto-UI   单独测   ->  正确
Proto-UI   在路径里 ->  "Proto-UUI"    (U 重复)
```

**单独测都对、组合起来就错**，且错法随上下文变化。这同时排除了"模型不认识这些词"和"反分词器坏了"。

## 判别性测试的设计要点（可复用）

排查这类问题时，**随机十六进制串**是最有价值的探针：
它无法被模型"纠正"成更常见的写法，所以能干净地把
**「数值/解码链路损坏」** 和 **「模型层面的行为问题」** 分开。

- 随机 hex 抄对 + 真实词抄错 → 模型行为问题，别再查算子
- 随机 hex 也抄错 → 才是数值链路问题

另一个要点：**`temperature=0` 下重复多次**。
确定性输出意味着同一输入必然同一输出，
所以"多次结果完全一致"直接排除一切随机性来源（NaN、量化抖动、采样）。

## 剩下的唯一可能与下一步

剩下的只有模型本身。手上有两个维度可以分开：
...
- **Cyber Q6_K** 对了 → 权重量化问题，升档即可
- **Cyber Q6_K** 还错 → 解限版微调损伤了逐字遵循能力，需要考虑换原版

（abliterated 类微调会削弱"严格按指令逐字复述"这类能力，是已知副作用。）

---

# 续五：真正的根因 —— 分词器没读 merges 排名（2026-08-20，已修复）

## 一行代码

`src/model.cpp` 的 GGUF 词表加载：

```cpp
model->weight.AddTokenizerWord(it.string_value(), idx, 1.0f);
//                                                     ^^^^ 248320 个 token 全是 1.0
```

而 BPE 的合并优先级正是按 score 排的（`src/tokenizer.cpp::TryMergePairs`）：

```cpp
q.push(SymbolPairs(now->score, l, r, ...));
// operator< : a.score < b.score || (a.score == b.score && a.l > b.l)
```

**score 全相等 → 比较退化成按位置 → 变成「从左到右贪心合并」，完全不看 merges 排名。**
那不是 BPE。

## 为什么这会让模型"胡言乱语"

规范 BPE 由 merges 的**排名**唯一定义：排名 0 优先级最高，合并点可以出现在词的
任何位置。GPT-2 系词表（`tokenizer.ggml.model == "gpt2"`）**没有 scores 字段**，
分词完全靠 merges —— 本模型的 GGUF 里确实只有 `tokens` / `token_type` / `merges`，
没有 `scores`。

拿 SentencePiece 的「按分数贪心」去套 GPT-2 词表，得到的是**合法但非规范**的切分：

```
规范:      brutalist -> ['br', 'ut', 'alist']
从左贪心:  brutalist -> ['bru', 'talist']       -> 模型复述成 "bru talist"
```

模型看到的 token 序列与训练时不同，逐字复述必然走样。

**这也是为什么只有本仓有这个问题**：llama.cpp / vLLM / HF 都实现了按排名的规范 BPE。

## 修法

第 `i` 条 merge 规则合成的 token，score 设为 `-i`。现有的最大堆自然优先弹出
排名最小（rank 0 → score 0）的合并，等价于规范 BPE，**算法骨架一行未动**。
非 merge 产生的 token（单字节等）给 `-1e9` 保证盖不过真实合并；
无 merges 的旧模型退回原行为，保持兼容。

启动日志新增：`Load tokenizer merges = 247587 (BPE 合并排名已启用)`
...
| 探针 | 修复前 | 修复后 |
|---|---|---|
| 完整路径（含 prototypes/brutalist） | ✗ 10/10 全错且错法一致 | ✓ |
| `brutalist` | ✗ `bru talist` | ✓ |
| `prototypes` | ✗ 在路径里为 `proto-types` | ✓ |
| `Proto-UI` | ✗ 在路径里为 `Proto-UUI` | ✓ |
| 随机十六进制 | ✓ | ✓ |
| **合计** | **0/10** | **6/6** |

## 这一晚的判断过程（值得记下的教训）

我先后把根因归到 **MTP 有损判据 → KV 反量化 → NaN → 反分词器 → 权重量化档位**，
每次都有证据支撑、每次都被证伪。真正的原因在分词器的**编码**侧 ——
一个在最早期就被排除、之后再没回去看的地方。

最终定位靠的是两件事，都不是"继续往数值链路里挖"：

1. **判别性探针**：随机十六进制串**无法被模型"纠正"**，所以
   「hex 抄对 + 真实词抄错」直接把「数值/解码链路损坏」和「切分/行为问题」分开。
2. **纯逻辑推理**：同一个词在不同上下文里**错法不同**。反分词器是纯函数，
   同样的 token 序列必然拼出同样结果 —— 上下文相关的错误只能来自
   **输入侧切分不同**。这一步不花 GPU。

另一条：`temperature=0` 下重复 10 次得到**完全一致**的错误，直接排除了一切
随机性来源（NaN、量化抖动、采样）。确定性错误必然来自确定性的错误算法。
