...
| **FastLLM Turbo3 分层 prefix cache** | GPU trie + zstd CPU/NVMe 冷页 + 动态成本门控 | cold 12.41s / GPU hit 2.01s / NVMe restore 2.84s | 16K 验收池；每请求逻辑上限独立 | 未验证 |
...
> FastLLM 当前推荐的 Turbo3 缓存/调度组合为 `--kv_cache_dtype turbo3 --tokens 262144 --batch 5`，并显式设置 `FASTLLM_QWEN35_TURBO3_KV=1`。16 个 full-attention 层使用 K=`Q8_0_KV`、V=`TURBO3_KV`；48 个 linear-attention/GDN 层保持 FP16 recurrent state。pool 内请求可 resident 调度，超池时可启用完整 CPU/disk swap；独立的 page-aligned prefix tier 已验证 GPU partial hit 与 zstd NVMe restore。首版 packed attention按 bounded chunk 解压到 FP16 后复用 cuBLAS，因此这是容量/命中率结果，不是低比特直接计算的吞吐声明。
